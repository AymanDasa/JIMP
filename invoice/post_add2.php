...
$MovePort = explode(',', $MovePort1);
$unmber_of_move = count($MovePort) ;

if($debug){echo "<b>MovePort1 : </b>".$MovePort1."<br>";}
if($debug){echo "<b>unmber_of_move : </b>".$unmber_of_move."<br>";}
    if( $MovePort1!="") 
        { 
            $MAX=MSP($ShipWeight,1);
            $MA0=floatval($unmber_of_move * $MAX[0])/2 ;
            $MA1=floatval($unmber_of_move * $MAX[1])/2 ;
            $MA2=floatval($unmber_of_move * $MAX[2])/2 ;
            $MA3=floatval($unmber_of_move * $MAX[3])/2 ;
            $MA=$MA0+$MA1+$MA2+$MA3;
            $XMove =$MovePort1;
