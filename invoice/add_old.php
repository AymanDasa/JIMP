        ...
        $AgentOption='<option value=""></option>';
				 	$query = "SELECT `Agent_ID`,`AgentNameAr`,`AgentCR` FROM `agents`;"; 
					$Agents = $dbop->query($query)->fetchAll();   
					foreach ($Agents as $Agents) {    
            $AgentOption.='<option value="'.$Agents['Agent_ID'].'">'.$Agents['AgentNameAr'].' CR:'.$Agents['AgentCR'].'
                              </option>';
					}
				// option Agents
        ...
        $ShipNameOption='<option value=""></option>';
				 	$query = "SELECT `Ship_ID`,`ShipName`  FROM `ship`;"; 
					$ships = $dbop->query($query)->fetchAll();   
					foreach ($ships as $ship) {    
            $ShipNameOption.='<option value="'.$ship['Ship_ID'].'">'.$ship['ShipName'].' 
                              </option>';
					}
				// option Ships
        ...
        // DUBLICAT ITEM
			...
			if(isset($_POST['save']))
			{
				...
				CHECK ALL INVOICES  AMOUNT
				
				*/
				if($_POST['invoiceamount1']){
					$invoiceamount1=  	stripslashes(htmlentities( strip_tags($_POST['invoiceamount1'] )));
					$A=1;} 
				if($_POST['invoiceamount2']){
					$invoiceamount2=  	stripslashes(htmlentities( strip_tags($_POST['invoiceamount2'] )));
					$C=1;} 
				if($_POST['invoiceamount3']){
					$invoiceamount3=  	stripslashes(htmlentities( strip_tags($_POST['invoiceamount3'] )));
					$E=1;} 
				if($_POST['invoiceamount4']){
					$invoiceamount4=  	stripslashes(htmlentities( strip_tags($_POST['invoiceamount4'] )));
					$G=1;} 
				if($_POST['invoiceamount5']){
					$invoiceamount5=  	stripslashes(htmlentities( strip_tags($_POST['invoiceamount5'] )));
					$I=1;}  
				/*
				
				CHECK ALL INVOICES  NUMBERS
				
				*/
			
				if($_POST['invoiceno1']){
					$invoiceno1=  	stripslashes(htmlentities( strip_tags($_POST['invoiceno1'] )));
					$B=1; } 
				if($_POST['invoiceno2']){
					$invoiceno2=  	stripslashes(htmlentities( strip_tags($_POST['invoiceno2'] )));
					$D=1; } 
				if($_POST['invoiceno3']){
					$invoiceno3=  	stripslashes(htmlentities( strip_tags($_POST['invoiceno3'] )));
					$F=1; } 
				if($_POST['invoiceno4']){
					$invoiceno4=  	stripslashes(htmlentities( strip_tags($_POST['invoiceno4'] )));
					$H=1; } 
				if($_POST['invoiceno5']){
					$invoiceno5=  	stripslashes(htmlentities( strip_tags($_POST['invoiceno5'] )));
					$J=1; }  
			...
			*/
			if($A && $B){ 
				$TotalInvoiceAmount=$TotalInvoiceAmount +$invoiceamount1 ;
				$NumberOfAllInvoice+=1;
				if( $C && $D){ 
					$TotalInvoiceAmount=$TotalInvoiceAmount+$invoiceamount2;
					$NumberOfAllInvoice+=1;
					if($E && $F){ 
						$TotalInvoiceAmount=$TotalInvoiceAmount+$invoiceamount3;
						$NumberOfAllInvoice+=1;
						if($G && $H){ 
							$TotalInvoiceAmount=$TotalInvoiceAmount+$invoiceamount4;
							$NumberOfAllInvoice+=1;
							if($I && $J){ 
								$TotalInvoiceAmount=$TotalInvoiceAmount+$invoiceamount5;
								$NumberOfAllInvoice+=1;		
							}
						}
					}
				...
				if($debug){ 
				echo "<br> Number  : ".$NumberOfAllInvoice . "<br>" ;
				echo "<br> Total  Amount : ".$TotalInvoiceAmount . "<br>" ;}
			}else {
				echo "ERROR DATA Not compleated"; 
			}

 			/*
				
				COMPAR Total Invoice Amount with SAPPO TOTAL AMOUNT 
				
			*/
				$sappo=  		stripslashes(htmlentities( strip_tags($_POST['sappo'] )));
				$sappo=  		intval($_POST['sappo']);  
				$query = "SELECT `balance`  FROM `sappo` WHERE `id`=$sappo;"; 
				$sappo_balance = $dbop->query($query)->fetchAll(); 
				if(isset($sappo_balance[0]['balance']))  {$sappo_balance = floatval( $sappo_balance[0]['balance'])  ; }
					else{echo '<div> 
						 Error: Invoice creation failed. Please check your input and try again. 
						 <br> The SAP balance is  0 <a href="index.php"> Back </a>
						</div>'; 
				...
				if($debug){  echo $sappo_balance; }
				if( ( $TotalInvoiceAmount > $sappo_balance ) || $NumberOfAllInvoice < 1){
					if($debug){ 
					echo " ERROR DATA Not compleated<br>"; 
					echo " Total Invoice Amount Grater than SAP PO Balance <br> ";
					echo " TotalInvoiceAmount  =  ".$TotalInvoiceAmount."<br>";
					echo " SAP PO Balance  =  ".$sappo_balance."<br>";
					echo " Number Of All Invoices  =  ".$NumberOfAllInvoice."<br>"; 
					}
					$DataIsOK='<div class="alert alert-danger alert-dismissible">
					<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
					 <i class="icon fas fa-check"></i> <B>Error</B>  <br>
					 
					 Error: Invoice creation failed. Please check your input and try again. 
					 <br> The SAP balance is  '.number_format($sappo_balance, 2, '.', ',').'
					</div>'; 
					 $OverBalance=1;  
				}else{ 
					$NewBalance = $sappo_balance - $TotalInvoiceAmount;
					$query1=" UPDATE `sappo` SET `balance` = ".$NewBalance." WHERE `sappo`.`id` = ".$sappo."; ";
					$result = $dbop->query($query1);
					if($NewBalance <= 0){
					$query2=" UPDATE `sappo` SET `is_active` = '0' WHERE `sappo`.`id` = ".$sappo."; ";
					$result2 = $dbop->query($query2);}
					if($debug){ 
						echo " ERROR DATA Not compleated<br>"; 
						echo " Total Invoice Amount Grater than SAP PO Balance <br> ";
						echo " TotalInvoiceAmount  =  ".$TotalInvoiceAmount."<br>";
					...
					$DataIsOK='<div class="alert alert-success alert-dismissible">
					<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
					 <i class="icon fas fa-check"></i> <B>Successful</B>  <br>
					Invoice creation was completed successfully. <br> The new balance is '  .number_format($NewBalance, 2, '.', ',').'
					</div>';  
				}
				...
				$ref_no=  	stripslashes(htmlentities( strip_tags($_POST['ref_no'] ))); 
				$supplier= 	stripslashes(htmlentities( strip_tags($_POST['supplier'] )));
				$schedule=	stripslashes(htmlentities( strip_tags($_POST['schedule'] )));
				$schedule= 	intval($schedule);


				$invoiceno1=  		stripslashes(htmlentities( strip_tags($_POST['invoiceno1'] ))); 
				$Invoicedate1=		stripslashes(htmlentities( strip_tags($_POST['Invoicedate1'] ))); 
				$invoiceamount1=  	stripslashes(htmlentities( strip_tags($_POST['invoiceamount1'] ))); 
				$invoiceamount1=  	floatval($invoiceamount1); 
				
				if($NumberOfAllInvoice){
					$query.=" (  NULL
							, '".$invoiceno1."' 
							, '".$ref_no."' 	
							, '".$Invoicedate1."'	
							...
							, ".$sappo."	
							, ".$schedule.")  ";}   
				if($NumberOfAllInvoice>=2){
					$invoiceno2=  		stripslashes(htmlentities( strip_tags($_POST['invoiceno2'] ))); 
					$Invoicedate2=  	stripslashes(htmlentities( strip_tags($_POST['Invoicedate2'] ))); 
					$invoiceamount2=  	stripslashes(htmlentities( strip_tags($_POST['invoiceamount2'] ))); 
					$invoiceamount2=  	floatval($invoiceamount2);  
					$query.=" ,(  NULL
							, '".$invoiceno2."' 
							, '".$ref_no."' 	
							, '".$Invoicedate2."'	
							...
							, ".$sappo."	
							, ".$schedule.")  "; }
				if($NumberOfAllInvoice>=3){
					$invoiceno3=  		stripslashes(htmlentities( strip_tags($_POST['invoiceno3'] ))); 
					$Invoicedate3= 	stripslashes(htmlentities( strip_tags($_POST['Invoicedate3'] ))); 
					$invoiceamount3=  	stripslashes(htmlentities( strip_tags($_POST['invoiceamount3'] ))); 
					$invoiceamount3=  	floatval($invoiceamount3);  
					$query.=" , (  NULL
							, '".$invoiceno3."' 
							, '".$ref_no."' 	
							, '".$Invoicedate3."'	
							...
							, ".$sappo."	
							, ".$schedule.") "; } 
				if($NumberOfAllInvoice>=4){
					$invoiceno4=  		stripslashes(htmlentities( strip_tags($_POST['invoiceno4'] ))); 
					$Invoicedate4=  	stripslashes(htmlentities( strip_tags($_POST['Invoicedate4'] ))); 
					$invoiceamount4=  	stripslashes(htmlentities( strip_tags($_POST['invoiceamount4'] ))); 
					$invoiceamount4=  	floatval($invoiceamount4);  
					$query.=" , (  NULL
							, '".$invoiceno4."' 
							, '".$ref_no."' 	
							, '".$Invoicedate4."'	
							...
							, ".$sappo."	
							, ".$schedule.") "; }
				if($NumberOfAllInvoice>=5){
					$invoiceno5=  		stripslashes(htmlentities( strip_tags($_POST['invoiceno5'] ))); 
					$Invoicedate5=  	stripslashes(htmlentities( strip_tags($_POST['Invoicedate5'] ))); 
					$invoiceamount5=  	stripslashes(htmlentities( strip_tags($_POST['invoiceamount5'] ))); 
					$invoiceamount5=  	floatval($invoiceamount5);  
					$query.=" , (  NULL
							, '".$invoiceno5."' 
							, '".$ref_no."' 	
							, '".$Invoicedate5."'	
						...
						<div class="row">
							<div class="col-sm-12 col-md-4 col-lg-4 col-xl-3"> 
								...
								<div class="form-group">
									...
									<select name="sappo" class="form-control select2">
										<?=$ShipNameOption;?>
									</select>
								</div> 
							</div> 
							...
							<div class="col-sm-12 col-md-4 col-lg-4 col-xl-3"> 
								...
								<div class="form-group">
									...
									<div class="input-group date" id="ClickInvoiceDate" data-target-input="nearest">
										<input  name="ClickInvoiceDate" type="text" class="form-control datetimepicker-input" data-target="#ClickInvoiceDate"/>
						...
						<div class="row">
								...
								<div class="col-sm-12 col-md-3 col-lg-3 col-xl-3">  
									<div class="form-group">
										...
										<div class="input-group date" id="ClickArrivalDate" data-target-input="nearest">
											<input  name="InvoiceDate" type="text" class="form-control datetimepicker-input" data-target="#ClickArrivalDate"/>
							...
							<!-- %%  To Date  %%% --> 
								...
								<div class="input-group date" id="ClickDepartureDate" data-target-input="nearest">
									<input type="text" class="form-control datetimepicker-input" data-target="#ClickDepartureDate"/>
							...
							<!-- %%  Partial/Final  %%% -->     
								<div class="col-sm-12 col-md-3 col-lg-3 col-xl-3"> 
									<div class="form-group">
										...
										<input type="text" class="form-control" name="MovePort1">
									</div>
								</div>
						</div>  
						<!-- %%%%%%%%%%%%%%%%%%%% row3   %%%%%%%%%%%%%%%%%%%%%%%% -->    
						<div class="row">
								...
								<div class="col-sm-12 col-md-3 col-lg-3 col-xl-3">  
									<div class="form-group">
										...
										<div class="input-group date" id="ClickAnchorEntryDate" data-target-input="nearest">
											<input  name="InvoiceDate" type="text" class="form-control datetimepicker-input" data-target="#ClickAnchorEntryDate"/>
							...
							<!-- %%  To Date  %%% --> 
								...
								<div class="input-group date" id="ClickAnchorLeaveDate" data-target-input="nearest">
									<input type="text" class="form-control datetimepicker-input" data-target="#ClickAnchorLeaveDate"/>
							...
							<div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">   
								<table id="example1" class="table table-bordered table-striped">
									...
									<tbody>
										...
										<tr>
											<td>
												<div class="input-group date" id="Invoicedate<?php echo $li;?>" data-target-input="nearest">
													<input type="text" name="Invoicedate<?php echo $li;?>" class="form-control datetimepicker-input" data-target="#Invoicedate<?php echo $li;?>"/>
													<div class="input-group-append" data-target="#Invoicedate<?php echo $li;?>" data-toggle="datetimepicker">
														<div class="input-group-text">
															<i class="fa fa-calendar"></i>
														</div>
													</div>
											...
											<td>
												<div class="form-group">
													<label></label>
													<input type="text" class="form-control" name="invoiceno<?php echo $li;?>"  autocomplete="off">
												</div>
											</td>
											<td>
												<div class="form-group">
													<label></label>
													<input type="text" class="form-control" name="invoiceamount<?php echo $li;?>"  autocomplete="off">
												</div>
											</td>
										</tr>
										<?php }?>
...
$(function () {
    ...
    $('#Invoicedate4').datetimepicker({
    format: 'YYYY-MM-DD'
    });
    $('#Invoicedate5').datetimepicker({
    format: 'YYYY-MM-DD'
    });
